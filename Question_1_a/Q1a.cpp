...
int main()
{
    ...
    real pi,dx,X_l,X_u,Tolerance,dt,T_end,Alpha,m;
    ...
    for (int n = 0;n < Value_of_N_s.size();n++)
    {
        ...
        for (int t_steps = 0; t_steps < num_steps; t_steps++ )
        {
            for (int j = 1;j < N-1;j++)
            {
                ...
                if (u_n[j] < 0)
                {
                    u_n_1[j] = u_n[j]*(1 - (m*(u_n[j+1] - u_n[j])));
                }
                else
                {
                    u_n_1[j] = u_n[j]*(1 - (m*(u_n[j] - u_n[j-1])));
                }
            }
            ...
            if (t_steps%30 == 0)
            {
                write_to_file(u_n_1,"Question_1_a_u_n_t_"+to_string(t_steps*dt)+"_N_"+to_string(N)+"_.csv");
                Output_file_names.push_back("Question_1_a_u_n_t_"+to_string(t_steps*dt)+"_N_"+to_string(N)+"_.csv");
            }
        }
        ...
        for (int j = 1;j < N-1;j++)
            {
                ...
                if (u_n[j] < 0)
                {
                    u_n_1[j] = u_n[j]*(1 - (m_new*(u_n[j+1] - u_n[j])));
                }
                else
                {
                    u_n_1[j] = u_n[j]*(1 - (m_new*(u_n[j] - u_n[j-1])));
                }
            }
        ...
        write_to_file(u_n_1,"Question_1_a_u_n_t_"+to_string(T_end)+"_N_"+to_string(N)+"_.csv");
        Output_file_names.push_back("Question_1_a_u_n_t_"+to_string(T_end)+"_N_"+to_string(N)+"_.csv");
    }
